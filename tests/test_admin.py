...
def test_form_inputs_view(client):
...
def test_equations_view(client):
...
def test_knowledge_base_view(client):
...
def test_export_view(client, setup_system):
...
def test_no_file_uploaded(client, setup_system):
...
def test_valid_db_upload(client, setup_empty_database):
